...
class bcn_breadcrumb_trail
{
	...
	public function __construct()
	{
		...
		$this->opt = array(
			...
			'Hsearch_template' => sprintf(
				'<span property="itemListElement" typeof="ListItem"><span property="name">%1$s</span><meta property="position" content="%%position%%"></span>',
				sprintf(
					esc_attr__('Search results for &#39;%1$s&#39;', 'breadcrumb-navxt'),
			...
			'Hsearch_template_no_anchor' => sprintf(
			...
			'Hauthor_template' => sprintf(
				'<span property="itemListElement" typeof="ListItem"><span property="name">%1$s</span><meta property="position" content="%%position%%"></span>',
				sprintf(
					esc_attr__('Articles by: %1$s', 'breadcrumb-navxt'),
			...
			'Hauthor_template_no_anchor' => sprintf(
				'<span class="%%type%%">%1$s</span>',
	...
	protected function type_archive($type, $type_str = false)
	{
		...
		//Have a taxonomy, try to figure the type out from that
			...
			isset($type->taxonomy) && isset($GLOBALS['wp_taxonomies'][$type->taxonomy]->object_type[0])
	...
	public function fill()
	{
		...
		//For archives
			...
			//If we have a post type archive, and it does not have a root page generate the archive
				...
				is_post_type_archive() && !isset($type->taxonomy)
	...
	protected function display_loop($breadcrumbs, $linked, $reverse, $template, $outer_template, $separator, $depth = 1)
	{
		...
		foreach ($breadcrumbs as $key => $breadcrumb) {
			...
			if (is_array($breadcrumb)) {
				$trail_str .= sprintf(
					$outer_template,
					$this->display_loop($breadcrumb, $linked, $reverse, $template, $outer_template, $this->opt['hseparator_higher_dim'], $depth + 1),
					$separator
				);
