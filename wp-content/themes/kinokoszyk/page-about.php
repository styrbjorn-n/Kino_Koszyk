...
<section class="font-heading mx-2 md:mx-16 lg:mx-[120px]">
    ...
    <?php if (have_posts()) : while (have_posts()) : the_post(); ?>
            ...
            <section class="flex flex-row justify-between">
                <div class="basis-1/2">
                    <h2 class="mb-6 font-heading text-desktopH2"><?php the_field('kino_koszyk_header') ?></h2>
                ...
                <div class="hidden md:block md:basis-[40%]">
                    <div class="w-full h-full max-md:mt-4">
                        ...
                        <img class="object-fill mb-4 w-full" src="<?= esc_url($thumb); ?>" alt="<?= esc_attr($alt);  ?>" />

                        <?php $image = get_field('about_image_2');
                        if ($image) {
                            $title = $image['title'];
                            $alt = $image['alt'];
                            $size = 'large';
            ...
            <section class="flex flex-row justify-between ">
                <div class="basis-2/5">
                    ...
                    <?php $image = get_field('bo_persson_image');
                    if ($image) {
                        $title = $image['title'];
                        $alt = $image['alt'];
                        $size = 'large';
                ...
                <div class="basis-2/5">
                    ...
                    <?php $image = get_field('joanna_helander_image');
                    if ($image) {
                        $title = $image['title'];
                        $alt = $image['alt'];
                        $size = 'large';
